  METHOD download_registered_taxpayers4.
...
*    CLEAR ls_user_list.
*    FIND ALL OCCURRENCES OF '</eFaturaKayitliKullanici>' IN lv_taxpayers_xml RESULTS DATA(lt_matches).
*    IF lt_matches IS NOT INITIAL.
*      DATA(lv_index) = lines( lt_matches ) / 2.
*      READ TABLE lt_matches INTO DATA(ls_match) INDEX lv_index.
*      IF sy-subrc = 0.
*        ls_match-offset += 26.
*        lv_taxpayers_xml2 = lv_taxpayers_xml+ls_match-offset(*).
*        lv_taxpayers_xml2 = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' &&
*                             '<eFaturaKayitliKullaniciListe>' &&
*                              lv_taxpayers_xml2.
*        lv_taxpayers_xml = lv_taxpayers_xml(ls_match-offset) &&
*                           '</eFaturaKayitliKullaniciListe>'.
*        CALL TRANSFORMATION zetr_inv_userlist_efn
*          SOURCE XML lv_taxpayers_xml
*          RESULT efaturakayitlikullaniciliste = ls_user_list.
*        CALL TRANSFORMATION zetr_inv_userlist_efn
*          SOURCE XML lv_taxpayers_xml2
*          RESULT efaturakayitlikullaniciliste = ls_user_list2.
*        APPEND LINES OF ls_user_list2-efaturakayitlikullanici TO ls_user_list-efaturakayitlikullanici.
*        CLEAR: ls_user_list2, lt_matches.
*      ENDIF.
*    ENDIF.
